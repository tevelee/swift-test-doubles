...
| ABI-uncertain imported results | Automatic compiler-proven transport covers methods, indexed getters, forwarding spies, supported function-value results, and tuple leaves returning built-in Foundation values; explicit adapters or generated compiled fallbacks cover custom results | Unproven custom leaves and typed-error buffers remain fail-closed. |
...
discovery with a `GetterEffect.throwing` hint and preserve successful, untyped
error, and concrete typed-error results. The getter hint supplies the missing
effect bit while its resilient or witness symbol retains the exact typed-error
metadata and dynamic-`Self` index identity. Instance methods support
borrowed/default, consuming, synchronous, async, untyped-throwing, and
supported typed-throwing forms. The declaring protocol determines the ABI: an
ordinary protocol uses opaque indirect storage, while a protocol that itself
requires `AnyObject` uses one direct reference word. A class-constrained child
does not change an inherited unconstrained base requirement's ABI.
Configuration and invocation use generic-opening helper functions because an
existential cannot call a `Self`-taking requirement directly. Explicit
schemas, Spies, superclass-constrained existentials, initializers, async static
methods, `inout`, nested optionals, and other wrappers remain fail-closed.
...
counts are never negative, so negative range bounds simply cannot match an
observed call count.

The `verify(_:within:_:)` overload accepts a lower-bounded range such as `2...`
and waits without polling. Calls wake the waiter through the recorder's
...
that contains a compiler-proven Foundation leaf is decomposed recursively,
including mixed direct and caller-owned indirect result members. A tuple with
an unproven ABI-uncertain leaf remains unsupported because Swift lowers its
elements independently. Optional and nominal wrappers around the same tuple
remain one whole, calibratable value.
...
other scalar types. Automatic and linked mangled-type discovery both resolve
SIMD generic metadata directly (`SIMD2` through `SIMD64`, over any concrete
`SIMDScalar`), the same as any other supported shape; explicit
`.method(signatureOf:)` requirements remain available but are no longer
...
bounded async stack path also accepts complete SIMD spills occupying one
through four registers as described below. Smaller or padded vectors, values
needing more than four vector registers, partial vector spills, nested or
...
verification; async dispatch copies every element before suspension. The same
standalone shape may forward through a synchronous or async Spy.
...
suspension, the entry bridge removes the compiler-planned, ABI-aligned outgoing
stack reservation exactly once.
...
the forwarded call. The target metadata/witness-table pair must follow them on
the same stack path.
...
to eight such words as explicit parameters, never through `TDCallFrame`, so
...
register is orthogonal to stack transport. A ninth spilled word, split or
