...
@Suite struct IntegrationStyleTests {
    ...
    // Disabled: PaymentResult (String + Double + Bool ≈ 32 bytes) exceeds the 16-byte threshold
    // for register returns. RuntimeStub's W1 thunks assume ≤ 16-byte return values; larger structs
    // require an sret (indirect-return) calling convention not yet generated by the thunk scripts.
    @Test(.disabled("sret ABI: PaymentResult > 16 bytes, thunks only support ≤ 16-byte returns"))
