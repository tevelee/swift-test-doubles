...
protocol AssociatedTypedThrowingProbe<Failure> {
    associatedtype Failure: Error
    func load(_ shouldFail: Bool) throws(Failure) -> Int
}
struct RealAssociatedTypedThrowingProbe: AssociatedTypedThrowingProbe {
    func load(_ shouldFail: Bool) throws(ThrowingProbeError) -> Int { 1 }
}
protocol ExplicitAssociatedTypedThrowingProbe<Failure> {
    associatedtype Failure: Error
    func load(_ shouldFail: Bool) throws(Failure) -> Int
}
struct WrappedAssociatedTypedError<Value>: Error {}
protocol WrappedAssociatedTypedThrowingProbe<Failure> {
    associatedtype Failure: Error
    func load() throws(WrappedAssociatedTypedError<Failure>) -> Int
}
struct RealWrappedAssociatedTypedThrowingProbe:
    WrappedAssociatedTypedThrowingProbe
{
    func load() throws(WrappedAssociatedTypedError<ThrowingProbeError>) -> Int { 1 }
}
...
@Suite struct BoundAssociatedTypeTests {
    ...
    @Test func throwingDependentRequirementsPropagateResultsAndErrors() throws {
        ...
        stub.verify(.exactly(2)) { try $0.transform(Match.any()) }
    }
...
private func exerciseDependentSetter(
    ...
    #expect(weakReference.value != nil)
    stub.verify { $0.value = Match.any(using: placeholder) }
...
private func exerciseConsumingDependentArguments(
    ...
    await stub.verify { await $0.consumeAsync(Match.any(using: placeholder)) }
