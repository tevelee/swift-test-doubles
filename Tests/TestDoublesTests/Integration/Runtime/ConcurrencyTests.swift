// WASI has no libdispatch, and this whole suite exists to exercise a custom
// DispatchQueue-backed SerialExecutor, so there is no meaningful subset of it
// to keep: skip it entirely on that platform rather than rewrite tests whose
// point is Dispatch interop to not use Dispatch.
